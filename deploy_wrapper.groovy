def installApps(deployScript, config) {
   ...
   withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'SPRING_CLOUD_CONFIG_USER', usernameVariable: 'CONFIG_USERNAME', passwordVariable: 'CONFIG_PASSWORD'],
    ...
    unarchive mapping: ['**/*.war,**/*.jar,TODO.md' : '.']
    ...
    parallel (
      "yoo-actuator-test": {
        ...
        deployScript.blueGreenDeployment(config.mainCfDomain, env.CONFIG_PASSWORD, env.ENCRYPT_KEY, config.apiAppName, config.springProfile, 'web.manifest.yml', './yooture.web/target/yooture.web.jar', config.apiPublicDomains, config.apiServicesToBind, config.apiInstances)
      },
      failFast: true
    )
